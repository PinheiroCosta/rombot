...
async def clear(ctx, limit=100):
    ...
    await ctx.channel.send(
            f"Deletando minhas mensagens do canal {ctx.channel.mention} ...")

    sleep(2)
    deleted = await ctx.channel.purge(
            limit=limit,
            check=lambda msg: not msg.pinned and msg.author == bot.user)

    await ctx.channel.send(
            f"{len(deleted)} mensagens foram excluídas", delete_after=3)
...
async def clear_all(ctx, limit=100):
    ...
    await ctx.channel.send(
            f"Deletando mensagens do canal {ctx.channel.mention} ...")
    sleep(2)
    deleted = await ctx.channel.purge(
            limit=limit,
    ...
    await ctx.channel.send(
            f"{len(deleted)} mensagens foram excluídas",
            delete_after=3)
...
async def kick(ctx, member: discord.Member, *, reason="eu quis"):
    ...
    await ctx.channel.send(
            f"{member} não está mais no servidor porque {reason}")


@kick.error
async def kick_info_error(ctx, error):
...
@nick_edit.error
async def nick_info_error(ctx, error):
    if isinstance(error, commands.CommandInvokeError):
